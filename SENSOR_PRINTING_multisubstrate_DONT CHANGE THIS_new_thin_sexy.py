...
def print_cover(z, height, length, over, speed, pressure, start, valve = 1):
    ...
    ##PDMS cover for reference cantilever
    g.meander(x=length/2.5, y=height*1.2, spacing = over, start=start, orientation = 'y')
...
def print_all_covers(nozzle = 'A', valve = 0):
    for i in range(0,4):
        ...
        g.clip(axis=nozzle, direction='-y', height=8)
    ...
    for i in range(4,8):
        ...
        g.move(x=0.5, y=0.5)
        g.abs_move(**{nozzle:0.23})
        print_cover(z=0.3, height=4.9, length = 11.5, over = 0.4, speed = 12, pressure = cover_pressure[i], start='LL', valve = valve)
        g.clip(axis=nozzle, direction='+y', height=8)
...
reference_nozzle = 'A'
active_slide = 'slide1'
z_ref = -80.190877 #slide1
#active_slide = 'slide2'
#z_ref = -80.196254#slide2
...
#g.set_home(x=0, y=0)
#
#print_sacrificial(trace_speed = 5, height = -0.1, over = 0.75, nozzle = 'C', overhang = 0)
...
#print_all_wires_no_stop(valve='2',nozzle='B',initial_dwell=2)
#g.toggle_pressure(pressure_box)
