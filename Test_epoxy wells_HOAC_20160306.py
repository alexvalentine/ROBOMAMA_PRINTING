...
insulating_height_bot = (0.015,)*16 #slide1
...
align_top_pressure=(17.5,)*16#(10,)*16
...
############# POLYAMIDE INK 
electrode_height=0.12
electrode_pressure = 23
electrode_speed = 8
...
def print_sacrificial(trace_speed, height, over, nozzle, overhang = 0, y_inset = cant_y_translate):
    ...
    for i in range(1,8,2):
        g.feed(15)
        g.abs_move(x=cantilever_position[i][0]-0.375, y=cantilever_position[i][1]-y_inset -0.5)
        g.move(x=-overhang)
        meander_2tails(x=(cantilever_width + 2*overhang)+0.75, y=-(cantilever_length-y_inset), spacing=over,
    ...
    for i in range(9,16,2):
        ...
        g.feed(15)
        g.abs_move(x=cantilever_position[i][0]-0.375, y=cantilever_position[i][1] +0.5 + y_inset)
        g.move(x=-overhang)
        meander_2tails(x=(cantilever_width + 2*overhang)+0.75, y=(cantilever_length - y_inset), spacing=over, 
                    z=height, tail = 0.5, clip_direction = '+y', speed=trace_speed, 
                    orientation = 'y', nozzle = nozzle, valve = None)
...
def print_all_wires_no_stop(valve, nozzle,initial_dwell):
    ...
    g.set_pressure(pressure_box, wire_pressure[0])
    g.move(x=(-tail+inset)-3, y=extra)
    g.abs_move(**{nozzle:wire_height[0]})
    g.feed(8)
    for i in range(0,8,2):
        j=i/2
        g.abs_move(x=cantilever_position[i][0]+(-tail+inset), y = cantilever_position[i][1]+extra)
        #g.set_pressure(pressure_box, wire_pressure[i/2])
        if valve is not None:
            g.set_valve(num = valve, value = 1)
        print_wires_no_stop(z=wire_height[j], speed=wire_speed[j], extra = extra,
            tail = 1.5, width = width, length=(cantilever_length - 0.75), valve = valve, nozzle = nozzle, clip_direction = '+y', k=j, arc_direction = 'CCW')
        g.set_valve(num = valve, value = 0)
    ...
    for i in range(8,16,2):
        ...
        if valve is not None:
            g.set_valve(num = valve, value = 1)        
        print_wires_no_stop(z=wire_height[j], speed=wire_speed[j], extra = extra, 
            tail = 1.5, width = width, length=-(cantilever_length - 0.75), valve = valve, nozzle = nozzle, clip_direction = '-y', k=j, arc_direction = 'CW')
        g.set_valve(num = valve, value = 0)
        g.move(x=1.5,**{nozzle:2})
        if i==14:
            print ''
...
def print_insulating_layer(nozzle, valve, TorB, bottom_shift):
    if TorB == 'T':
    ...
    for i in range(0,8):
        g.abs_move(cantilever_position[i][0] + bottom_shift, cantilever_position[i][1] - y_translation)
        if valve is not None:
            g.set_valve(num = valve, value = 1)
...
###
active_slide = 'slide1'
z_ref = -75.550356#automator.substrate_origins[active_slide]['A'][2]#-69.558571
####
#active_slide = 'slide2'
#z_ref = -69.546409
...
#set_home_in_z()
#g.abs_move(x=automator.substrate_origins[active_slide]['A'][0] - 1, y=automator.substrate_origins[active_slide]['A'][1] - 0.5)
#g.set_home(x=0, y=0)
#
#g.abs_move(x=0, y=0)
...
#g.set_home(x=0, y=0)
#
#print_sacrificial(trace_speed = 10, height = -0.075, over = 0.375, nozzle = 'C', overhang = 0)
...
#
#print_insulating_layer(valve='1',nozzle='A',TorB='B',bottom_shift = 0)
##print_insulating_layer(valve='1',nozzle='A',TorB='B',bottom_shift = 0.065)  #####second bottom TPU layer, shifted over a small bit
#g.toggle_pressure(pressure_box)
...
#print_insulating_layer(valve='1',nozzle='A',TorB='T',bottom_shift = 0)
#g.toggle_pressure(pressure_box)
...
print_all_single_wells(layer_height = 0.3, layer_increments=5, total_increments=6, pressure=90, speed=14, nozzle = 'B', valve = 0)
